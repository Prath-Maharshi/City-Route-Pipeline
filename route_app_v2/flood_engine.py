...
def _load_flood_polygons(
    ...
    """Parse GeoJSON flood file; cache by (path, threshold)."""
    with open(fp) as f:
        fc = json.load(f)
...
def flood_capacity_factor(depth: float) -> float | None:
    ...
    if depth >= FLOOD_DEPTH_HARD:  return None   # full remove_edge
    if depth >= FLOOD_DEPTH_MED:   return 0.10
    if depth >= FLOOD_DEPTH_LIGHT: return 0.40
    return 1.0
